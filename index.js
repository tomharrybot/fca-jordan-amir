...
try {
    ...
    if (!global.Fca.Require.fs.existsSync(process.cwd() + '/SHONA.json')) {
        global.Fca.Require.fs.writeFileSync(process.cwd() + "/SHONA.json", JSON.stringify(global.Fca.Data.ObjPriyansh, null, "\t"));
        process.exit(1);
    }

try {
    var Data_Setting = require(process.cwd() + "/SHONA.json");
}
catch (e) {
    global.Fca.Require.logger.Error('Detect Your SHONA Settings Invalid!, Carry out default restoration');
    global.Fca.Require.fs.writeFileSync(process.cwd() + "/SHONA.json", JSON.stringify(global.Fca.Data.ObjPriyansh, null, "\t"));     
    process.exit(1)
}
    if (global.Fca.Require.fs.existsSync(process.cwd() + '/SHONA.json')) {
        ...
        for (let i of All_Variable) {
            if (Data_Setting[i] == undefined) {
                ...
                global.Fca.Require.fs.writeFileSync(process.cwd() + "/SHONA.json", JSON.stringify(Data_Setting, null, "\t"));
            }
            else continue; 
        } //Check Variable
            ...
            if (global.Fca.Require.FastConfig.Stable_Version.Accept == true) {
                if (Data.Stable_Version.Valid_Version.includes(global.Fca.Require.FastConfig.Stable_Version.Version)) {
                    ...
                    log.warn("[ FCA-UPDATE ] •", "Error Stable Version, Please Check Your Stable Version in FastConfig.json, Automatically turn off Stable Version!");
                        ...
                        global.Fca.Require.fs.writeFileSync(process.cwd() + "/SHONA.json", JSON.stringify(global.Fca.Require.FastConfig, null, "\t"));
                    process.exit(1);
                }
            }
            else {
